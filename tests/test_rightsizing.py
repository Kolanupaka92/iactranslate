...
def test_rule_engine_rightsize_uses_catalog():
    ...
    assert instance_exists(s.instance_type)
    assert s.ami_key == "windows-2022"
...
def test_build_compute_plans_all_valid():
    ...
    plans = build_compute_plans(vms, provider, tier_env)
    assert len(plans) == 2
    for p in plans:
        assert instance_exists(p.instance_type)
        assert p.estimated_monthly_cost_usd > 0
    db = next(p for p in plans if p.vm_name == "prod-db-01")
    assert db.extra_volumes_gib == [500]  # first disk is root, rest are data volumes
