...
@pytest.mark.parametrize("target_name", ["aws", "azure", "gcp", "oci", "digitalocean"])
...
def test_targets_pick_distinct_instance_families(rvtools_path):
    ...
    do_types = {c.instance_type for c in build_migration_plan(vms, "t", get_target("digitalocean")).compute}
    ...
    assert all(t.startswith(("s-", "m-")) for t in do_types)
