"""Nutanix AHV target — VMs, VLAN subnets and categories via the nutanix provider.

The first on-premises destination. Deliberately carries no `CAP_PRICED`: an
AHV VM's cost is hardware, licensing and facilities, which no inventory can
...
class NutanixTarget:
    name = "nutanix"
    #: Not a region — there are none on-premises. Carried as the cluster name
    #: so the shared `region` plumbing still has something meaningful to show.
    default_region = "cluster-01"
