"""DigitalOcean target — Droplet/VPC/Firewall via Terraform digitalocean provider.

Capabilities are narrower than AWS/Azure/GCP, like OCI: no live pricing
integration and no Pulumi renderer exist yet, so `capabilities` only
advertises what's actually implemented.
"""
...
class DigitalOceanTarget:
    ...
    def sg_for_tier(self, tier: Tier) -> str:
        return mapping.SG_BY_TIER.get(tier, "app-fw")
