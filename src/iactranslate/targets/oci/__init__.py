"""OCI target — Compute/VCN/NSG via Terraform oci provider.

Capabilities are deliberately narrower than AWS/Azure/GCP: no live pricing
integration and no Pulumi renderer exist for OCI yet, so `capabilities` only
advertises what's actually implemented (CAP_TERRAFORM, CAP_GITOPS) rather than
claiming support that isn't there.
"""
